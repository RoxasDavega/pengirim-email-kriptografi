...
import { detailSentMail } from '../templates/template-creator';
...
const SentDetail = {
  ...
  async afterRender() {
    ...
    const detailedMail = await EmailDbSource.getDetailSentMail(url.id);
    const userData = JSON.parse(localStorage.getItem('auth'));
    document.querySelector('.greeting').innerHTML = `Hello, ${userData.fullName}`;
    console.log(detailedMail);

    mailContainer.innerHTML += detailSentMail(detailedMail);
